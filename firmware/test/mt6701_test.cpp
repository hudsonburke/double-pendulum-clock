 ...
 * 
 * Purpose: Minimal initialization and continuous angle readout for manual verification
 * Protocol: SPI/SSI (Serial Synchronous Interface, NOT standard SPI MODE)
 * Resolution: 14-bit (0-16383 raw = 0-360°)
 * 
 * IMPORTANT: MT6701 uses SPI_MODE2 (CPOL=1, CPHA=0)
 * Clock is idle HIGH, data captured on falling edge
 * 
...
#define MT6701_SPI_MODE SPI_MODE2 // Clock idle HIGH, sample on falling edge
...
void setup() {
  ...
  Serial.println("Multiple columns: RAW (0-16383), DEGREES (0-360), CUMULATIVE (multiple turns)\n");
...
uint16_t mt6701_readRawAngle() {
  ...
  SPI.beginTransaction(SPISettings(MT6701_SPI_SPEED, MSBFIRST, MT6701_SPI_MODE));
  ...
  // Top 8 bits from data[0]
  // Bottom 6 bits from upper 6 bits of data[1]
  uint16_t raw = ((uint16_t)data[0] << 6) | ((data[1] >> 2) & 0x3F);
 ...
 *   11 = invalid
 */
uint8_t mt6701_getStatus(uint8_t byte1) {
  return byte1 & 0x03;
}
